            ...
            switch ($this->state) {
                ...
                default:
                    if ($this->tagChange($this->ctag, $this->ctagLen, $text, $i)) {
                        ...
                        if ($this->tagType !== self::T_DELIM_CHANGE) {
                            // check for filters
                            $newBuffer = explode(self::T_FILTER_DELIMITER, trim($token[self::NAME]), 2);
                            if (count($newBuffer) === 2) {
                                $token[self::FILTERS] = trim($newBuffer[1]);
                            }
                            $token[self::NAME] = $newBuffer[0];

                            // check for attributes
                            $newBuffer = preg_split('/[\s]/', trim($token[self::NAME]), 2);
                            if (count($newBuffer) === 2) {
                                $token[self::ATTRS] = $newBuffer[1];
                            }
                            $token[self::NAME] = $newBuffer[0];
                        }
